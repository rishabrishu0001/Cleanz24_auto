...
export default function CityServiceForm({ cityName, serviceName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = isFranchiseQuery ? {
        ...
        source: `Franchise City Service Page - ${cityName} - ${serviceName}`, Source: `Franchise City Service Page - ${cityName} - ${serviceName}`
      } : {
        timestamp: dateStr,
        name: form.name,
        mobile: `'${form.phone}`,
        email: form.email || 'N/A',
        service: `${serviceName} Booking (${cityName})`,
        date: 'Asap',
        time: 'Anytime',
        address: cityName,
        type: 'City Service Page Booking Request',
        source: `Franchise City Page - ${cityName} - ${serviceName}`,
      ...
      if (typeof window !== "undefined") {
        if (typeof window.gtag === "function") {
          ...
          window.gtag("event", "laundry_service_city_lead", { event_category: "ServiceBooking", event_label: `${cityName} - ${serviceName}` });
        }
      }

      setSubmitted(true);
    } catch (err) {
      console.error("Error submitting city-service form:", err);
  ...
  if (submitted) {
    return (
      <div style={{ textAlign: 'center', padding: '30px 10px', color: '#4ade80' }}>
        ...
        <h4 style={{ fontWeight: '700', marginBottom: '8px', color: '#fff' }}>Booking Received!</h4>
        <p style={{ fontSize: '0.92rem', color: '#94a3b8', lineHeight: '1.6' }}>
          Thank you! Our operations and franchise team in <strong>{cityName}</strong> will call you within 24 hours regarding your booking for <strong>{serviceName}</strong>.<br /><br />
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      <div>
        ...
        >
          ...
          <option value="Franchise Opportunity">Open a franchise store in {cityName}</option>
        </select>
      </div>
