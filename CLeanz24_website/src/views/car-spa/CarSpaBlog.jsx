  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        >
          <Link href={`/car-spa/blog/${post.slug}`}
      ...
      <div className="container" style={{ position: 'relative', zIndex: 2 }}>
        ...
        <nav aria-label="breadcrumb" style={{ marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', fontSize: '13px', color: isDarkMode ? 'rgba(255,255,255,0.6)' : 'rgba(0,0,0,0.6)', alignItems: 'center' }}>
            <Link href="/car-spa" style={{ color: isDarkMode ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.7)', textDecoration: 'none' }}>Car Spa</Link>
            <span>›</span>
    ...
    if (!post) {
      return (
        <div style={{ background: isDarkMode ? '#0a0a0a' : '#f7fafc', minHeight: '100vh', color: 'var(--text-main)', padding: '100px 20px', textAlign: 'center' }}>
          ...
          <Link href="/car-spa/blog" className="btn btn-outline-success px-4 py-2">Back to Blog</Link>
        </div>
      );
    }
      ...
      <div style={{ background: 'var(--bg-body)', minHeight: '100vh', color: 'var(--text-main)', transition: 'background 0.3s ease, color 0.3s ease' }}>
        ...
        <section
          style={{
            position: 'relative',
            height: '45vh',
            backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, ${isDarkMode ? 'rgba(5,7,5,1)' : '#f7fafc'} 100%), url(${post.image?.src || post.image})`,
