...
const storeLocationsGrouped = [
  ...
  { state: 'Maharashtra', cities: ['Alibag', 'Pune', 'Thane West', 'Wakad'] },
...
export default function Stores() {
  const { isDarkMode } = React.useContext(CarSpaThemeContext);
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <nav aria-label="breadcrumb" style={{ marginBottom: '30px', textAlign: 'center' }}>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', fontSize: '13px', color: isDarkMode ? 'rgba(255,255,255,0.7)' : '#4A5568', alignItems: 'center' }}>
              <Link href="/car-spa" style={{ color: isDarkMode ? 'var(--primary-color)' : '#2B6CB0', textDecoration: 'none', fontWeight: 600 }}>Home</Link>
              <span>›</span>
                    ...
                    <div className="mb-3 pt-3 border-top" style={{ borderColor: isDarkMode ? 'var(--card-border)' : '#f1f5f9' }}>
                      ...
                      <div className="store-guides-section p-2 rounded mb-2" style={{ 
                        ...
                        <div className="d-flex flex-column gap-1">
                          <Link href={`/car-spa/blog/best-car-spa-nearby-you-${store.city.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${store.id}`}
                            className="text-decoration-none text-brand-primary fw-semibold"
                          >
                            • Best Car Spa Nearby
                          </Link>
                          <Link href={`/car-spa/blog/best-ceramic-coating-in-${store.city.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${store.id}`}
                            className="text-decoration-none text-brand-primary fw-semibold"
                          >
                            • Ceramic Coating Guide
                          </Link>
                          <Link href={`/car-spa/blog/best-ppf-paint-protection-film-in-${store.city.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${store.id}`}
                            className="text-decoration-none text-brand-primary fw-semibold"
                          >
                            • PPF Detailing Guide
                          </Link>
