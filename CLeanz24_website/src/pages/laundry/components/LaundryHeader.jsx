    ...
    <>
      ...
      >
        <div className="container">
          {/* Logo */}
          <Link className="navbar-brand d-flex align-items-center text-decoration-none gap-2" to="/laundry">
          ...
          {isMobile ? (
            ...
            /* Desktop Links */
            <div className="d-none d-lg-flex navbar-collapse justify-content-end">
              <div className="navbar-nav align-items-center">
                {navLinks.filter(l => !l.accent).map(link => (
                  <Link
                    key={link.to}
                    className={`nav-link text-heading mx-2 fw-medium ${isActive(link.to) || (link.to !== '/laundry' && location.pathname.startsWith(link.to)) ? 'active-link' : ''}`}
                    to={link.to}
                  >
                    {link.label}
                  </Link>
          ...
          <>
            ...
            >
              ...
              <div className="laundry-panel-header">
                <Link to="/laundry" onClick={() => setMobileMenuOpen(false)}>
                  <img src={logoImg} alt="Cleanz24" style={{ height: '40px', borderRadius: '6px' }} />
                </Link>
